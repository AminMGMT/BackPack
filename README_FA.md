...
**بک‌پک** یک هستهٔ تونل معکوس (Reverse Tunnel) با کارایی بالاست که کاملاً با **Go**
نوشته شده و برای ست‌آپ سرور ایران ⇄ خارج طراحی شده. یک باینری واحد است با یک منوی
تعاملی CLI **و** یک پنل وب امن — یعنی همه‌چیز را با ترمینال یا بدون ترمینال
می‌توانی مدیریت کنی.
...
```
  کاربر ──▶  سرور ایران  ══ تونل ══▶  سرور خارج  ──▶  سرویس واقعی
             «Setup Server»            «Setup Client»    (X-UI، پنل،
...
|------|-----|------------|-----|
| **ایران** | ورودی | **۱. Setup Server** | پورت‌ها را باز می‌کند؛ کاربر به **آی‌پی ایران** وصل می‌شود. |
| **خارج** | خروجی | **۲. Setup Client** | به سرور ایران وصل می‌شود و ترافیک را به سرویس واقعی می‌رساند. |
