...
func (c *WsTransport) poolMaintainer() {
	...
	for {
		...
		case <-tickerLoad.C:
			...
			// outside is indistinguishable from a leak. Every other pooled
			// transport publishes what its pool is doing and ws did not, so
			// the panel's connection-pool card was simply missing on a ws or
			// wss tunnel — not empty, not zero, absent — while the same card
			// appeared for every other transport.
			//
			// The throughput figure is real here: ws counts its bytes through
			// AddBytes, because it reads and writes messages rather than
			// handing out a net.Conn.
			metrics.ReportPool(poolConnectionsAvg, newPoolSize, c.config.ConnPoolSize, load.mbps())
